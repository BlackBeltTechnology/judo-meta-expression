        ...
        modelAdapter.getAttributes(entityType)
                .forEach(attribute -> {
                    ...
                    if (!modelAdapter.isDerivedAttribute(attribute)) {
                        ...
                        boolean hasDefaultBinding = hasAttributeBinding(typeName, modelAdapter.getName(attribute).get(), AttributeBindingRole.DEFAULT);
                        JqlExpressionBuilder.BindingContext defaultBindingContext = new JqlExpressionBuilder.BindingContext(modelAdapter.getName(attribute).get(), JqlExpressionBuilder.BindingType.ATTRIBUTE, JqlExpressionBuilder.BindingRole.DEFAULT);
                        ...
                        Optional<String> defaultJql = modelAdapter.getAttributeDefault(attribute);
                        ...
                        if (!hasDefaultBinding) {
                            defaultJql.ifPresent(jql -> {
                                buildAndBind(entityType, null, defaultBindingContext, jql);
                            });
                        } else {
        ...
        modelAdapter.getReferences(entityType)
                .forEach(reference -> {
                    ...
                    if (!modelAdapter.isDerivedReference(reference)) {
                        ...
                        boolean hasDefaultBinding = hasReferenceBinding(typeName, modelAdapter.getName(reference).get(), ReferenceBindingRole.DEFAULT);
                        boolean hasRangeBinding = hasReferenceBinding(typeName, modelAdapter.getName(reference).get(), ReferenceBindingRole.RANGE);
                        JqlExpressionBuilder.BindingContext defaultBindingContext = new JqlExpressionBuilder.BindingContext(modelAdapter.getName(reference).get(), JqlExpressionBuilder.BindingType.RELATION, JqlExpressionBuilder.BindingRole.DEFAULT);
                        JqlExpressionBuilder.BindingContext rangeBindingContext = new JqlExpressionBuilder.BindingContext(modelAdapter.getName(reference).get(), JqlExpressionBuilder.BindingType.RELATION, JqlExpressionBuilder.BindingRole.RANGE);
                        ...
                        Optional<String> defaultJql = modelAdapter.getReferenceDefault(reference);
                        Optional<String> rangeJql = modelAdapter.getReferenceRange(reference);
                        ...
                        if (!hasDefaultBinding) {
                            defaultJql.ifPresent(jql -> buildAndBind(entityType, null, defaultBindingContext, defaultJql.get()));
                        } else {
                            log.debug("Default expression already extracted for navigation property: {}", reference);
                        }

                        if (!hasRangeBinding) {
                            rangeJql.ifPresent(jql -> buildAndBind(entityType, null, rangeBindingContext, rangeJql.get()));
                        } else {
